...
extension Highlight {
    ...
    public static func findByHighlightId(_ highlightId: String) -> Highlight? {
        var highlight: Highlight?
        
        let predicate = NSPredicate(format: "highlightId = %@", highlightId)
        let realm = try! Realm()
        let highlights = realm.objects(Highlight.self).filter(predicate).toArray(Highlight.self)
