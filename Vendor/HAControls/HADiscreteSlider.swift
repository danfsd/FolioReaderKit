...
//	}
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
		self.touchDown(touches as NSSet, duration: 0.1)
	}
	
    override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
		self.touchDown(touches as NSSet, duration: 0.0)
	}
	
    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        self.touchUp(touches as NSSet)
	}
	...
	func touchDown(_ touches: NSSet, duration: TimeInterval) {
		let touch = touches.anyObject() as? UITouch
        if let touch: UITouch = touch {
			let location = touch.location(in: touch.view)
			self.moveThumbTo(location.x, duration: duration)
		}
	}
	
	func touchUp(_ touches: NSSet) {
		let touch = touches.anyObject() as? UITouch
        if let touch: UITouch = touch {
			let location = touch.location(in: touch.view)
